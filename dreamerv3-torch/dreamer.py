...
# Choose default MUJOCO_GL only if the user did not set one.
# macOS rarely has OSMesa; fall back to glfw there.
...
def make_env(config, mode, id):
    ...
    if suite == "dmc":
        ...
        import envs.audio as audio

        env = audio.AudioToy(
            size=tuple(config.size),
            audio_dim=config.audio_dim if config.audio_dim > 0 else 16,
            horizon=config.time_limit,
            seed=config.seed + id,
        )
    elif suite == "audiovision":
        import envs.audio_image as audio_image

        env = audio_image.AudioVisionToy(
            size=tuple(config.size),
            audio_dim=config.audio_dim if config.audio_dim > 0 else 16,
            horizon=config.time_limit,
            seed=config.seed + id,
