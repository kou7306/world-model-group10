...
class AddAudioObs(gym.Wrapper):
    """Injects synthetic audio observation when the environment does not provide one."""

    def __init__(self, env, dim=16, noise_std=0.0):
        super().__init__(env)
        self._dim = dim
        self._noise_std = noise_std
        spaces = dict(self.env.observation_space.spaces)
        if "audio" not in spaces:
            spaces["audio"] = gym.spaces.Box(
                -np.inf, np.inf, shape=(self._dim,), dtype=np.float32
            )
        self.observation_space = gym.spaces.Dict(spaces)

    def _generate_audio(self):
        audio = np.zeros((self._dim,), dtype=np.float32)
        if self._noise_std > 0:
            audio += np.random.normal(
                0.0, self._noise_std, size=(self._dim,)
            ).astype(np.float32)
        return audio
    ...
    def step(self, action):
        ...
        if "audio" not in obs:
            obs = obs.copy()
            obs["audio"] = self._generate_audio()
        return obs, reward, done, info
