...
```
Run training with dummy audio modality (adds synthetic `audio` obs of dim 16 via wrapper):
```
python3 dreamer.py --configs dmc_vision --task dmc_walker_walk --logdir ./logdir/dmc_walker_walk_audio --audio_dim 16 --audio_noise_std 0.01
```
Use `--audio_dim 0` (default) when the environment natively provides `audio` in its observations.
Run training on the built-in toy audio env (audio vector only, blank image):
```
python3 dreamer.py --configs audio_toy --logdir ./logdir/audio_toy
```
Run training on the built-in audio+vision toy env (audio vector + colored square image):
```
python3 dreamer.py --configs audio_vision_toy --logdir ./logdir/audio_vision_toy
```
Monitor results:
```
