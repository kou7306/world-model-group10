...
class MultiEncoder(nn.Module):
    def __init__(
        ...
        self.audio_shapes = (
            {
                k: v
                for k, v in shapes.items()
                if len(v) in (1, 2) and audio_keys and re.match(audio_keys, k)
            }
            if audio_keys
            else {}
        )
        self.mlp_shapes = {
            ...
            and k not in self.audio_shapes
        }
        print("Encoder CNN shapes:", self.cnn_shapes)
        print("Encoder Audio shapes:", self.audio_shapes)
        ...
        if self.audio_shapes:
            input_size = sum([sum(v) for v in self.audio_shapes.values()])
            self._audio_mlp = MLP(
                input_size,
                None,
                audio_layers or mlp_layers,
                audio_units or mlp_units,
                act,
                norm,
                symlog_inputs=symlog_inputs,
                name="AudioEncoder",
            )
            self.outdim += audio_units or mlp_units
        if self.mlp_shapes:
    ...
    def forward(self, obs):
        ...
        if self.audio_shapes:
            inputs = torch.cat([obs[k] for k in self.audio_shapes], -1)
            outputs.append(self._audio_mlp(inputs))
        if self.mlp_shapes:
...
class MultiDecoder(nn.Module):
    def __init__(
        ...
        print("Decoder Audio shapes:", self.audio_shapes)
    ...
    def forward(self, features):
        ...
        if self.audio_shapes:
            dists.update(self._audio_mlp(features))
        if self.mlp_shapes:
